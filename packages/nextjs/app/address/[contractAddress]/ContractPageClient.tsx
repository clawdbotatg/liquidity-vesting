...
const VESTING_ABI = deployedContracts[8453].LiquidityVesting.abi;
...
export default function ContractPage({ contractAddress }: { contractAddress: `0x${string}` }) {
  ...
  const { data: contractOwner } = useReadContract({ address: vestingAddress, abi: VESTING_ABI, functionName: "owner" });
  const { data: vestedLiquidityData } = useReadContract({
    address: vestingAddress,
    abi: VESTING_ABI,
    functionName: "vestedLiquidity",
    query: { refetchInterval: 3000 },
  });
  const { data: initialLiquidityData } = useReadContract({
    address: vestingAddress,
    abi: VESTING_ABI,
  ...
  const { writeContractAsync: writeClaimRaw, isPending: claimMining } = useWriteContract();
  const writeClaim = ({ functionName }: { functionName: string }) =>
    writeClaimRaw({ address: vestingAddress, abi: VESTING_ABI, functionName: functionName as "claim" });
  const { writeContractAsync: writeVestRaw, isPending: vestMining } = useWriteContract();
  const writeVest = ({ functionName, args }: { functionName: string; args: [bigint, bigint] }) =>
    writeVestRaw({ address: vestingAddress, abi: VESTING_ABI, functionName: functionName as "vest", args });
  const { writeContractAsync: writeClaimAndVestRaw, isPending: claimAndVestMining } = useWriteContract();
  const writeClaimAndVest = ({ functionName, args }: { functionName: string; args: [bigint, bigint] }) =>
    writeClaimAndVestRaw({
      address: vestingAddress,
      abi: VESTING_ABI,
      functionName: functionName as "claimAndVest",
      args,
    });
  ...
  const writeLockUp = ({
    ...
    functionName: string;
    args: [bigint, bigint, bigint, number, number, bigint, bigint];
  }) => writeLockUpRaw({ address: vestingAddress, abi: VESTING_ABI, functionName: functionName as "lockUp", args });
