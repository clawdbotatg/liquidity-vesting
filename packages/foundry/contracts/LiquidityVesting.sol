...
// Inline Uniswap V3 math (solc 0.8 compatible)
...
/// @notice Minimal TickMath subset (from Uniswap V3, adapted for solc 0.8)
library TickMath {
...
/// @notice Minimal FullMath (from Uniswap V3, adapted for solc 0.8)
library FullMath {
...
/// @notice Minimal LiquidityAmounts (from Uniswap V3 periphery, adapted for solc 0.8)
library LiquidityAmounts {
    function getAmount0ForLiquidity(uint160 sqrtRatioAX96, uint160 sqrtRatioBX96, uint128 liquidity) internal pure returns (uint256) {
        if (sqrtRatioAX96 > sqrtRatioBX96) (sqrtRatioAX96, sqrtRatioBX96) = (sqrtRatioBX96, sqrtRatioAX96);
        return FullMath.mulDiv(uint256(liquidity) << 96, sqrtRatioBX96 - sqrtRatioAX96, sqrtRatioBX96) / sqrtRatioAX96;
    }

    function getAmount1ForLiquidity(uint160 sqrtRatioAX96, uint160 sqrtRatioBX96, uint128 liquidity) internal pure returns (uint256) {
        if (sqrtRatioAX96 > sqrtRatioBX96) (sqrtRatioAX96, sqrtRatioBX96) = (sqrtRatioBX96, sqrtRatioAX96);
        return FullMath.mulDiv(liquidity, sqrtRatioBX96 - sqrtRatioAX96, 0x1000000000000000000000000);
    }
...
contract LiquidityVesting is Ownable {
    ...
    function previewVest() public view returns (uint256 amount0, uint256 amount1) {
        ...
        (,,,,, int24 tickLow, int24 tickUp, uint128 liquidity,,,,) = positionManager.positions(tokenId);
        ...
        uint160 sqrtRatioLower = TickMath.getSqrtRatioAtTick(tickLow);
        uint160 sqrtRatioUpper = TickMath.getSqrtRatioAtTick(tickUp);

        (amount0, amount1) = LiquidityAmounts.getAmountsForLiquidity(
            sqrtPriceX96, sqrtRatioLower, sqrtRatioUpper, toLiquidate
        );
    }
